...
struct node *del(struct node *root, int dkey) {
    ...
    else if (ptr->lthread == false && ptr->rthread == false)  // Case with two children
        root = case_c(root, par, ptr);
    else if (ptr->lthread == false)  // Case with only left child
        root = case_b(root, par, ptr);
    else if (ptr->rthread == false)  // Case with only right child
        root = case_b(root, par, ptr);
    else  // Case with no children
        root = case_a(root, par, ptr);

    return root;
}

// Case when the node to be deleted has no children
struct node *case_a(struct node *root, struct node *par, struct node *ptr) {
...
// Case when the node to be deleted has one child
struct node *case_b(struct node *root, struct node *par, struct node *ptr) {
